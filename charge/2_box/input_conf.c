...
/////////////// 3) Name of output .gro                                  /////
/////////////// 4) Number of atoms per molecule of wall                 /////
/////////////// 5) Number of molecules of 1st element in bulk           /////
/////////////// 6) Number of atoms per molecules of 1st element in bulk /////
...
int main(int argc, char *argv[]){
	...
	int   atom,g,f,nmol,nbulk1,nbulk2,tbulk1,tbulk2,nbulk,abulk;
	...
	FILE *in,*in2,*in3,*in4;//declaring a poiter to a file
	...
	fscanf(in,"%s%s%s",string,string,string);//read file in, %s = string of characters, string name --> read name of wall  
	...
	n = 2*aux + aux2;// total number of particles = wall+bulk
	...
	fscanf(in4,"%s%s%s",string,string,string);//read file in, %s = string of characters, string name --> read name of wall
        fscanf(in4,"%d",&n1);//read file in, %d = decimal integer,variable --> read number of wall particles

	for(i=1;i<=aux;i++){
		fscanf(in4,"%d%6s%5s%5d%f%8f%8f\n",&k,string1,string2,&atom,&rx[i],&ry[i],&rz[i]);
		//read wall file for all n1 particle lines while alocates variables for each of the columns--> no velocities yet
		k    = k+nbulk;
		atom = atom+abulk;
           	fprintf(in3,"%5d%-5s%5s%5d%8.3f%8.3f%8.3f\n",k,string1,string2,atom,rx[i],ry[i],rz[i]+boxz1+boxz2+0.02+0.28+0.28);
                //write the information on output file for graphene
        }

	
	
	fprintf(in3,"%10.5f %10.5f %10.5f\n",boxx1,boxy1,boxz1+boxz2+0.3+boxz1+0.3);
